...
for i in range(1,36):
    ...
    questionfile.write(f"Bạn có 15 phút để hoàn thành bài thi của mình.\n MÃ ĐỀ{i}.ljust(20," ")\n ")
    questionfile.write("Phần bài làm:".center(80," "))
    questionfile.write('\n\n')
    answerfile.write("Phần đáp án cho câu hỏi".center(30,"-"))
    ...
    for x in range(50):
        ...
        #print(f"Câu hỏi {x+1}. Thủ đô của {question[x]} là gì?\n")
        answerfile.write(f'Đáp án câu {x+1} là:    {optionsanswer[optionsanswer.index(correctanswer)]}\n')
        for i in range(4):
            questionfile.write(f"\t{'ABCD'[i]}.{optionsanswer[i]}\n")
            #if i == 0:
            #    print(f'Đáp án câu {x+1} là: {optionsanswer[optionsanswer.index(correctanswer)]}\n')
            #print(f"\t{'ABCD'[i]}.{optionsanswer[i]}\n")
